...
#' @param t2_tiplab_size Update Tree 2 tip label font-size. Default 3.
#' @param t2_tiplab_pad Add spacing between Tree 2 tip label and Tree 2 tip ending. Default 0.
#'
#' @return None
#'
#' @examples
...
#'                   bootstrap_symbol = "*", t2_pad = 1, tiplab = TRUE)
#'
#'
#' @export
  ...
  if (!is.null(bootstrap_nodes) && nrow(bootstrap_nodes) > 0) {
    pp <- pp +
      geom_text(
        ...
        aes(x = x, y = y, label = .data$.bootstrap_symbol),
        inherit.aes = FALSE,
        size = bootstrap_size,
